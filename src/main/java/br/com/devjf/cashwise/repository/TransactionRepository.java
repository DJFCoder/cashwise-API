...
public interface TransactionRepository extends JpaRepository<Transaction, Long> {
    ...
    /**
     * Verifica se existem lançamentos vinculados a uma categoria. Útil antes de
     * excluir categorias para evitar inconsistências.
     *
     ...
     * @return lista de arrays contendo [mês, ano, total receitas, total
     * despesas]
     */
    ...
    // ========== QUERIES PARA CONTROLE DE RECORRÊNCIA ==========
    /**
     * Busca lançamentos originais (parent) com recorrência ativa. Usado pelo
     * RecurrencyJob para processar automaticamente.
     *
     * Critérios: - parentTransactionId IS NULL (é lançamento original) -
     * recurrencyActive = true (está ativo) - recurrency != UNIQUE (possui
     * recorrência)
     *
    ...
    /**
     * Busca o último lançamento filho gerado a partir de um lançamento
     * original. Usado para calcular a próxima data de recorrência.
     *
    ...
    /**
     * Conta quantos lançamentos filhos foram gerados a partir de um lançamento
     * original. Útil para estatísticas e controle.
     *
     * @param parentId ID do lançamento original (parent)
    ...
    /**
     * Busca todos os lançamentos filhos de um lançamento original. Útil para
     * exibir histórico completo de uma recorrência.
     *
     * @param parentId ID do lançamento original (parent)
    ...
    /**
     * Verifica se um lançamento já possui lançamentos filhos gerados. Útil
     * antes de permitir certas operações no lançamento original.
     *
     * @param parentId ID do lançamento original (parent)
    ...
    /**
     * Conta quantos lançamentos filhos foram criados após uma data/hora
     * específica.
     * <p>
     * Utilizado principalmente para controlar o limite de geração de
     * lançamentos recorrentes (máximo 2 por dia), garantindo que não ocorra
     * loop infinito no processamento de recorrências.
     * </p>
     * <p>
     * Exemplo de uso:
     * <pre>{@code
     * long criadosHoje = repository.countByParentTransactionIdAndCreatedAtAfter(
     *         parentId, LocalDate.now().atStartOfDay());
     * if (criadosHoje >= 2) {
     *     // Não gera mais filhos hoje
     * }
     * }</pre>
     * </p>
     *
     * @param parentId ID do lançamento original (parent) cuja descendência será
     * contada
     * @param since data/hora de referência - apenas lançamentos criados
     * <b>após</b> esse instante serão contados
     * @return quantidade de lançamentos filhos criados após o instante
     * fornecido
     */
    @Query("SELECT COUNT(t) FROM Transaction t WHERE t.parentTransactionId = :parentId AND t.createdAt >= :since")
    Long countByParentTransactionIdAndCreatedAtAfter(@Param("parentId") Long parentId, @Param("since") LocalDateTime since);
}
